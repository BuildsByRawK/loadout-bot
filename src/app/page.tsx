...
export default function Home() {
  ...
  const features = [
    ...
    },
    {
      ...
      description:
        "Filter loadouts by Meta, Mid-Tier, or Budget for clarity."
    },
    {
      title: "📖 Help Menu",
      ...
      description:
        "Accessible help guide for all commands and tips."
    }
  ];
      ...
      <section className="w-full max-w-6xl grid gap-6 md:grid-cols-3 px-2">
        {features.map((feature, index) => {
          ...
          return (
            ...
            >
              <h2 className={`${feature.color} text-xl font-semibold mb-2 flex items-center w-full justify-between`}>
                <span className="text-2xl text-left">{feature.title.slice(0, 2)}</span>
                <span className="flex-1 text-center">{feature.title.slice(2)}</span>
                <motion.span
                  initial={false}
